...
def production():
    ...
    env.hosts = ['data.dailyillini.com']
    env.user = 'projects'
    env.s3_bucket = 'media.dailyillini.com'
...
def create_database(func=run):
    ...
    func('echo "CREATE EXTENSION postgis;" | psql %(project_name)s' % env)
...
def load_data():
    ...
    local("scp %(localpath)s/data/dump.sql.gz %(user)s@data.dailyillini.com:data/%(project_name)s" % env)
